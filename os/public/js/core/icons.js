// Centralized icon library. Each icon is a small inline SVG.
// Usage: icons.get('terminal') -> <svg>...</svg>

const SVG = (inner, size = 24, stroke = 2) => `
...
const ICONS = {
  ...
  package: '<line x1="16.5" y1="9.4" x2="7.55" y2="4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/>',
  ...
  restore: '<rect x="8" y="8" width="13" height="13" rx="2"/><path d="M3 16V5a2 2 0 0 1 2-2h11"/>',
  ...
  moreV: '<circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" cy="5" r="1"/><circle cx="12" cy="19" cy="19" r="1"/>',
...
export function get(name, { size = 18, stroke = 2 } = {}) {
  const inner = ICONS[name];
  if (!inner) return '';
  return SVG(inner, size, stroke);
}

export function art(name, size = 48) {
  const inner = ICONS[name];
  if (!inner) return '';
  return `<div class="app-icon-art" style="width:${size}px;height:${size}px;display:flex;align-items:center;justify-content:center;border-radius:12px;background:linear-gradient(135deg,#7c5cff,#22d3ee);color:#fff;">${SVG(inner, Math.round(size * 0.5), 2.2)}</div>`;
}
